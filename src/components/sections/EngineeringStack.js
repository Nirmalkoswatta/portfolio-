...
const EngineeringStack = () => {
  ...
  return (
    <section id="architecture" className="section-padding relative overflow-hidden bg-slate-100/60 dark:bg-slate-950/40 border-y border-slate-200 dark:border-slate-900">
      <div className="container-custom">
        ...
        <div className="mb-12">
          ...
          <h2 className="heading-lg tracking-tight uppercase text-slate-900 dark:text-white">
            END-TO-END <span className="text-blue-600 dark:text-blue-400">ENGINEERING STACK</span>
          </h2>
            ...
            return (
              <motion.div
                ...
                className={`p-5 rounded-2xl border transition-all duration-300 cursor-pointer relative ${
                  isActive
                    ? 'bg-white dark:bg-slate-900 border-blue-500 shadow-xl shadow-blue-500/15'
                    : isCompleted
                    ? 'bg-white/80 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800/80 shadow-sm'
                    : 'bg-white/50 dark:bg-slate-900/40 border-slate-200 dark:border-slate-900'
                }`}
              >
                {/* Step number and status */}
                <div className="flex items-center justify-between mb-3">
                  <span className="font-mono text-xs text-slate-400 dark:text-slate-500">{step.num}</span>
                  {isActive ? (
                ...
                {/* Icon */}
                <div className={`p-2.5 rounded-xl w-fit mb-3 ${isActive ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'bg-slate-100 dark:bg-slate-800/60 text-slate-600 dark:text-slate-400'}`}>
                  <step.icon className="w-5 h-5" />
                </div>

                <h3 className="font-display font-bold text-xs text-slate-900 dark:text-white uppercase tracking-tight mb-1">
                  {step.title}
                </h3>
                <span className="font-mono text-[10px] text-blue-600 dark:text-blue-400 block mb-2">{step.tool}</span>
                <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed font-sans">{step.desc}</p>
              </motion.div>
            );
          })}
        </div>
        ...
        <div className="mt-8 p-4 rounded-xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 font-mono text-xs shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-slate-700 dark:text-slate-300">
              Active Stage: <strong className="text-blue-600 dark:text-blue-400">{pipelineSteps[activeStep].title}</strong> ({pipelineSteps[activeStep].tool})
            </span>
          </div>
